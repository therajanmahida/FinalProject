...
public class EmployeeREST {
    ...
    @Path("/id/{param}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getEmployeeById(@PathParam("param") int _employee_id){
        EmployeeDAO employeeDAO = new EmployeeDAO();
        List<EmployeeVO> list = employeeDAO.getEmployeeVOList();
        if(list.size() == 0) {
            return Response.status(Response.Status.NOT_FOUND).entity(gson.toJson("No Employee present")).build();
        }else{
            list = list.stream().filter(employeeVO -> employeeVO.getId() == _employee_id ).collect(Collectors.toList());
            if(list.size() == 0){
                return Response.status(Response.Status.NOT_FOUND).entity(gson.toJson("No Employee present with id = "+_employee_id)).build();
            }
            else{
    ...
    @GET
    @Path("/company/id/{param}/contact/{param2}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getEmployeeByContactAndCompany(@PathParam("param") int _company_id,@PathParam("param2") String contact_number){
        EmployeeDAO employeeDAO = new EmployeeDAO();
        List<EmployeeVO> list  = employeeDAO.getEmployeeVOListByCompany(_company_id);
        if(list.size() == 0 ){
            return Response.ok(gson.toJson("no")).build();
        }
        else{
            List<EmployeeVO> list1 = list.stream().filter(employeeVO -> employeeVO.getContactNumberOne().equals(contact_number)).collect(Collectors.toList());
            //return Response.ok(gson.toJson(list1)).build();
            list1.addAll(list.stream().filter(employeeVO -> employeeVO.getContactNumberTwo().equals(contact_number)).collect(Collectors.toList()));
            if(list1.size() == 0){
                return Response.ok(gson.toJson("no")).build();
            }
            else{
                return Response.ok(gson.toJson("yes")).build();
            }
        }
    }
    ...
    public Response insertEmployee(String employeeData) {
        ...
        return Response.ok(gson.toJson("Employee Inserted")).build();
    }
    ...
    public Response insertEmployeeList(String employeeDataList) {
        Type list_type = new TypeToken<ArrayList<EmployeeVO>>() {
        }.getType();
